...
function Ebupot2126UbahPph26() {
  ...
  useEffect(() => {
    getEbupot2126UbahPph26ById();
    getNegara();
    getObjekPajakData();
    getPenandatangan();
  }, []);

  const getEbupot2126UbahPph26ById = async () => {
    setOpenSearchIdentitasWp(true);
    const response = await axios.post(`${tempUrl}/eBupot2126Pph26s/${id}`, {
      _id: user.id,
      token: user.token,
    });
    // 01.) IDENTITAS WAJIB PAJAK YANG DIPOTONG
    ...
    setTinPasporKitasKitap(response.data.tinPasporKitasKitap);
    ...
    setNomorSkdWpln(response.data.nomorSkdWpln);

    // 03.) PENGHITUNGAN PPH PASAL 26
    setJumlahPenghasilanBruto(response.data.jumlahPenghasilanBruto);
    setTarif(response.data.tarif);
    setPPhYangDipotongDipungut(response.data.pPhYangDipotongDipungut);
  ...
  const updateEbupot2126UbahPph26 = async (e) => {
    e.preventDefault();
    e.stopPropagation();
    ...
    if (handlingInput) {
      try {
        setOpenSearchIdentitasWp(true);
        let updatedEBupot2126Pph26 = await axios.post(
          `${tempUrl}/updateEBupot2126Pph26/${id}`,
          {
            userId: user.id,
  ...
  const savedEbupot2126UbahPph26 = async (e) => {
    setOpenSaved(false);
          ...
          <Card style={{ marginTop: "20px" }}>
            ...
            <Card.Body>
              <Form
                noValidate
                validated={validated}
                onSubmit={updateEbupot2126UbahPph26}
              >
                <div>
...
export default Ebupot2126UbahPph26;
